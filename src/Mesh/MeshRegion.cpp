...
//    }
    ...
    template<int dim>
    typename MeshRegion<dim>::VectorDim
    MeshRegion<dim>::regionCenter() const
    {
        VectorDim temp(VectorDim::Zero());

        const auto& faceContainer = faces();

        if(faceContainer.empty())
        {
            return temp;
        }

        for(const auto& face : faceContainer)
        {
            temp += face.second->center();
        }

        return temp / faceContainer.size();
    }
